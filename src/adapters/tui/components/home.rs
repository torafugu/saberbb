...
use super::player_info::PlayerInfoWidget;
...
impl Home {
    pub fn new() -> Self {
        ...
        Self {
            ...
            team_schedule_state,
            team_schedule_month,
            player_info: PlayerInfoWidget::new(),
    ...
    fn draw_team_info(&mut self, frame: &mut Frame, area: Rect) -> color_eyre::Result<()> {
...
impl Component for Home {
    fn register_action_handler(&mut self, tx: UnboundedSender<Action>) -> color_eyre::Result<()> {
        ...
        self.pitching_stats.register_action_handler(tx.clone())?;
        self.player_info.register_action_handler(tx)?;
        Ok(())
    }
    ...
    fn register_config_handler(&mut self, config: Config) -> color_eyre::Result<()> {
        ...
        self.player_info.register_config_handler(config)?;
        Ok(())
    }
    ...
    fn update(&mut self, action: Action) -> color_eyre::Result<Option<Action>> {
        ...
        if matches!(self.selected_item, Some(MenuOption::ViewTeamInfo)) {
            if self.selected_team.is_some()
                ...
                && self.player_info.update_player_info(action.clone())
            {
                return Ok(Some(Action::Render));
            }
